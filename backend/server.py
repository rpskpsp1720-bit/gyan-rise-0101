...
def get_cors_origins() -> List[str]:
    """
    Parse the comma-separated CORS_ORIGINS env var into an exact allow-list.
    Each entry is normalized:
      - whitespace stripped
      - trailing slash removed
      - scheme + host lowercased
    Any entry containing a wildcard ('*') is excluded here and picked up by
    get_cors_origin_regex() below so it works via CORSMiddleware.allow_origin_regex.
    """
    raw = os.environ.get("CORS_ORIGINS", DEFAULT_FRONTEND_URL)
    result: List[str] = []
...
def get_cors_origin_regex() -> Optional[str]:
    """
    Build a single regex that CORSMiddleware.allow_origin_regex will match against.
    Sources:
      1. Any wildcard entries in CORS_ORIGINS (e.g., "https://*.vercel.app")
         — '*' is expanded to one URL label segment ([^./]+).
      2. CORS_ORIGIN_REGEX env var, if set (raw regex, appended as-is).
    Returns None if no patterns are configured (middleware simply skips the arg).
    """
    patterns: List[str] = []
    ...
    for entry in raw.split(","):
        ...
        if "*" not in s:
            continue
        # Escape everything, then turn each literal '*' into a URL-label wildcard.
        escaped = re.escape(s).replace(r"\*", r"[^./]+")
