...
/**
 * NoteViewer — in-app PDF viewer for chapter notes.
 * Mirrors DigitalPdfViewer exactly, but pulls the PDF stream from
 * /api/notes/{noteId}/preview instead of the digital-store endpoint.
 * Access is enforced on the backend via batch enrollment (no purchase step).
 */
export default function NoteViewer() {
  ...
  useEffect(() => {
    ...
    const fetchAndRender = async () => {
      setLoading(true);
      try {
        const res = await api.get(`/notes/${noteId}/preview`, { responseType: "arraybuffer" });
        const data = res.data;
  ...
  return (
    ...
    >
      <div className="p-2 bg-white border-b flex flex-wrap items-center gap-1 sm:gap-2 shrink-0">
        ...
        <div className="ml-1 text-xs sm:text-sm text-slate-600 whitespace-nowrap" data-testid="note-viewer-page-indicator">
          Page {pageNum} / {numPages}
        </div>

        <div className="ml-auto flex items-center gap-1 sm:gap-2 flex-wrap justify-end">
          <Button data-testid="note-viewer-zoom-out-btn" variant="outline" size="sm" onClick={zoomOut} title="Zoom out" aria-label="Zoom out">
            <ZoomOut className="h-4 w-4" />
          </Button>
          <div className="text-xs text-slate-600 w-11 text-center tabular-nums" data-testid="note-viewer-zoom-level">
            {Math.round(effectiveScale * 100)}%
          </div>
          <Button data-testid="note-viewer-zoom-in-btn" variant="outline" size="sm" onClick={zoomIn} title="Zoom in" aria-label="Zoom in">
            <ZoomIn className="h-4 w-4" />
          </Button>
          <Button
            data-testid="note-viewer-fit-width-btn"
          ...
          <Button
            data-testid="note-viewer-fit-page-btn"
          ...
          <Button
            data-testid="note-viewer-prev-page-btn"
            variant="outline"
            size="sm"
          ...
          <Button
            data-testid="note-viewer-next-page-btn"
            variant="outline"
            size="sm"
      ...
      <div
        ...
        data-testid="note-viewer-scroll"
        style={{
      ...
      >
        ...
        >
          <canvas
            ref={canvasRef}
            data-testid="note-viewer-canvas"
            style={{
              margin: "auto",
              display: "block",
              userSelect: "none",
